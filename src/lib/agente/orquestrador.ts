...
export function systemEstavel(): string {
  return [
    ...
    '- NUNCA DIGA "CONFIRMADO"/"AGENDADO"/"MARCADO" SEM TER CHAMADO A TOOL: isso vale pra agendar E agendar_revisao. O cliente respondeu "sim"? Isso autoriza você a CHAMAR a tool — não autoriza a declarar sucesso direto. A frase de confirmação só pode vir DEPOIS de rodar a tool NESTA MESMA resposta e ver um resultado de sucesso (com agendamento_id). Já aconteceu de responder "agendamento confirmado" sem nunca ter chamado agendar_revisao — o cliente ficou achando que tinha hora marcada e não tinha nada no sistema. Se a tool devolver erro (ex.: "precisa_atendente"), NUNCA diga que confirmou — explique o problema e use transferir_humano.',
