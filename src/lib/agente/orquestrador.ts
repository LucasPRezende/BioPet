...
export function systemEstavel(): string {
  return [
    ...
    '- O agendamento entra como PENDENTE, NÃO confirmado na hora: ao fechar um agendamento (agendar ou agendar_revisao) com sucesso, NÃO abra a mensagem final com "Agendamento confirmado!" — isso soa como uma certeza que ainda não existe (é só um pedido, a clínica ainda precisa confirmar). Use algo como "Agendamento solicitado ✓" ou "Pedido de agendamento registrado" no lugar disso, e deixe claro que a confirmação de verdade vem numa mensagem separada da BioPet (junto com o link de pagamento) — só aí está garantido. Se o cliente disser que já está na clínica ou vai entrar agora, avise que é melhor aguardar essa mensagem de confirmação antes de considerar certo.',
