...
// Gera o PDF do laudo de bioquímica APENAS para pré-visualização:
...
export async function POST(request: NextRequest) {
  ...
  let body: { pdfData: BioquimicaPDFData }
  try {
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        'Content-Disposition': 'inline; filename="previa_bioquimica.pdf"',
        'Cache-Control': 'private, no-store',
      },
    })
  } catch (err) {
    console.error('[preview-bioquimica] erro:', err)
