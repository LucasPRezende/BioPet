'use client'

import { useState } from 'react'
import Link from 'next/link'

type WaStatus = 'idle' | 'sending' | 'sent' | 'error'

interface Props {
  laudoId:    number
  tutor:      string
  telefone:   string
  titulo?:    string
  subtitulo?: string
  listaHref?: string
}

const WA_ICON = (
  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current shrink-0">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
)

export default function LaudoSucesso({
  laudoId,
  tutor,
  telefone,
  titulo    = 'Laudo cadastrado!',
  subtitulo,
  listaHref = '/admin/laudos',
}: Props) {
  const [waStatus, setWaStatus] = useState<WaStatus>('idle')

  async function sendWhatsApp() {
    setWaStatus('sending')
    try {
      const res = await fetch(`/api/laudos/${laudoId}/whatsapp`, { method: 'POST' })
      setWaStatus(res.ok ? 'sent' : 'error')
    } catch {
      setWaStatus('error')
    }
  }

  return (
    <div className="min-h-screen bg-[#19202d] flex items-center justify-center px-4">
      <div className="h-1 bg-gold-stripe absolute top-0 left-0 right-0" />
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg text-center">
        <div className="text-5xl mb-3">✅</div>
        <h2 className="text-2xl font-bold text-[#19202d] mb-1">{titulo}</h2>
        <p className="text-gray-400 text-sm mb-8">
          {subtitulo ?? `${tutor} · ${telefone}`}
        </p>

        <button
          onClick={sendWhatsApp}
          disabled={waStatus === 'sending' || waStatus === 'sent'}
          className={`flex items-center justify-center gap-2 w-full font-bold py-3 rounded-xl transition mb-3 ${
            waStatus === 'sent'    ? 'bg-green-100 text-green-700 cursor-default'
          : waStatus === 'error'   ? 'bg-red-50 text-red-600 border border-red-200'
          : waStatus === 'sending' ? 'bg-green-400 text-white cursor-wait'
          :                          'bg-green-500 hover:bg-green-600 text-white'
          }`}
        >
          {WA_ICON}
          {waStatus === 'sending' ? 'Enviando...'
            : waStatus === 'sent'  ? '✓ Enviado pelo WhatsApp!'
            : waStatus === 'error' ? 'Erro — tentar de novo'
            : 'Enviar pelo WhatsApp'}
        </button>

        <Link
          href={listaHref}
          className="flex items-center justify-center w-full border border-gray-200 hover:bg-gray-50 text-gray-500 px-4 py-2.5 rounded-xl transition text-sm"
        >
          Ver lista de laudos
        </Link>
      </div>
    </div>
  )
}
